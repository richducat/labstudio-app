...
async function seedDefaultProducts() {
  ...
  for (const p of defaults) {
    await q`
      insert into lab_products (slug, name, description, price_cents, checkout_url, image_url, active)
      values (${p.slug}, ${p.name}, ${p.description}, ${p.price_cents}, ${p.checkout_url}, ${p.image_url ?? null}, true)
...
export async function GET() {
  ...
  const products = (await q`
    select slug, name, description, price_cents, checkout_url, image_url, active
    from lab_products
    where active = true
