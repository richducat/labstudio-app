...
export default function SocialView() {
  ...
  return (
    <div className="pb-32 space-y-6">
      ...
      {/* Active Challenges */}
      <div className="px-1">
        <h3 className="font-semibold text-[10px] text-zinc-500 mb-3 uppercase tracking-widest ml-2">Featured Challenges</h3>
        <div className="space-y-3">
          {CHALLENGES.map((c) => (
            <Card key={c.id} className={`p-4 ${c.active ? 'border-violet-500/50 bg-violet-900/10' : 'opacity-70 grayscale'}`}>
              <div className="flex justify-between items-start mb-2">
                <div className="font-bold text-lg tracking-tight">{c.title}</div>
                <span className={`px-2 py-0.5 rounded text-[10px] font-semibold border uppercase tracking-wider ${c.active ? 'bg-violet-500/10 text-violet-400 border-violet-500/20' : 'bg-red-500/10 text-red-400 border-red-500/20'
                  }`}>
                  {c.active ? 'ACTIVE' : 'LOCKED'}
                </span>
              </div>
              <p className="text-xs text-zinc-400 mb-3 leading-relaxed">{c.desc}</p>
              <div className="flex items-center gap-2 text-[10px] font-semibold text-yellow-500 uppercase tracking-widest">
      ...
      <div className="px-1">
        <div className="flex items-center justify-between mb-3 px-2">
          <h3 className="font-semibold text-[10px] text-zinc-500 uppercase tracking-widest">Game Rankings</h3>
        ...
        <div className="space-y-2">
          {loading ? (
            ...
            leaderboard.map((u, i) => {
              ...
              <div
                ...
                className={`flex items-center gap-4 p-4 rounded-2xl transition-colors ${
                  isFirst 
                    ? 'bg-gradient-to-r from-yellow-500/20 to-zinc-900/50 border border-yellow-500/50 ring-1 ring-yellow-500/20' 
              ...
              >
                <div className={`font-semibold italic text-lg w-6 text-center ${isFirst ? 'text-yellow-500' : 'text-zinc-700'}`}>#{i + 1}</div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[9px] text-zinc-500 font-semibold uppercase tracking-[0.1em]">Member</div>
                </div>
                <div className="text-right">
                  <div className="font-mono font-semibold text-violet-400 text-sm">{(u.score || 0).toLocaleString()}</div>
            ...
            <div className="text-center py-12 px-8 bg-zinc-900/40 rounded-2xl border border-dashed border-zinc-800">
