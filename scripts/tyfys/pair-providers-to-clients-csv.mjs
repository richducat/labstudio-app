#!/usr/bin/env node
/**
 * Generate a CSV pairing pipeline clients → suggested providers.
 *
 * Source:
 * - Clients: Zoho Deals in stages Intake/Ready/Sent
 * - Providers: TYFYS portal directory (public)
 *
 * Output:
 * - CSV with one row per (client, need, provider suggestion)
 *
 * Usage:
 *   node scripts/tyfys/pair-providers-to-clients-csv.mjs --states FL,CA,CO,GA,TX \
 *     --maxNeeds 5 --maxProviders 3 \
 *     --out memory/provider-client-pairs-2026-02-04.csv
 */
...
(async function main() {
  ...
  const maxNeeds = Number(getArg('--maxNeeds', '5'));
  const maxProviders = Number(getArg('--maxProviders', '3'));
  const outPath = path.resolve(getArg('--out', `memory/provider-client-pairs-${new Date().toISOString().slice(0, 10)}.csv`));
  ...
  // Provider pool: portal providers filtered to these state names
  const ABBR_TO_NAME = {
  ...
  for (const d of targetDeals) {
    ...
    const needs = extractNeedTagsFromText(textBlob);
    const needsTop = needs.slice(0, maxNeeds);

    for (const need of needsTop) {
      const picks = pickProviders(providers, need, maxProviders);
      if (!picks.length) {
        // still include an empty row for visibility
        rows.push({
          client_name: d.Deal_Name,
          deal_id: dealId,
          stage: d.Stage,
          client_state: String(d.State || '').toUpperCase(),
          client_zip: d.Zip_code || '',
          need,
          provider_rank: '',
          provider_score: '',
          provider_name: '',
      ...
      for (const { p, score } of picks) {
        rows.push({
          ...
          provider_specialty: (p.specialty || []).join(' | '),
          provider_services: p.services || '',
          provider_email: p.email || '',
          provider_phone: p.phone || '',
          provider_contact: p.contact || '',
          provider_notes: p.notes || '',
          source: 'TYFYS portal',
        });
        rank++;
      }
  ...
  const header = [
    'client_name','deal_id','stage','client_state','client_zip','need',
    'provider_rank','provider_score','provider_name','provider_state','provider_county','provider_type',
    'provider_specialty','provider_services','provider_email','provider_phone','provider_contact','provider_notes','source'
  ];
  ...
  console.log(`Rows: ${rows.length} | Deals: ${targetDeals.length} | Providers (portal, in states): ${providers.length}`);
})().catch(err => {
